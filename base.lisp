...
(defun process-class-slot (slot)
  (labels ((member-any (items list)
             (labels ((test (x) (member x items)))
               (member-if #'test list))))
    (let ((name (first slot))
          (initform (unless (keywordp (second slot))
                      (second slot))))
      (let* ((slot (if initform
                       (cons (first slot) (cddr slot))
                       slot))
             (slot (if (not (member :initform slot))
    ...
    (let* ((option-start (position "&OPTIONS" args :key #'sym-only-name
      ...
      `(cl::defclass ,name ,direct-superclasses
         ,(mapcar #'process-class-slot
                  (mapcar (lambda (x) (if (listp x) x (list x))) direct-slots))
         ,@options))))
